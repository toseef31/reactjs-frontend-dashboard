...
const MediaGallery = ({ pennCatalogue_id }: MediaGalleryProps) => {
    const [penncataloguessMedia, setPennCataloguessMedia] = useState<any[]>([]);
    ...
    const getPennCataloguessMedia = async () => {
        try {
            const url = `${constants.BASE_URL}/penn-catalogues-media/${pennCatalogue_id}`;
            const response = await axios.get(url);
            setPennCataloguessMedia(response.data.data.media);
    ...
    const deleteMedia = async (id: number) => {
        ...
        try {
            const url = `${constants.BASE_URL}/penn-catalogues-media/delete`;
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files) {
            ...
            try {
                const url = `${constants.BASE_URL}/create-penn-catalogues-media`;
                ...
                getPennCataloguessMedia(); // Refresh media after upload
            } catch (err) {
    ...
    return (
        <div className="bg-white p-5 mt-3">
            <div className="text-lg font-semibold my-4">PennCataloguess Media</div>
            <div className="grid grid-cols-12 gap-2">
                {penncataloguessMedia.length > 0 ? (
                    penncataloguessMedia.slice().reverse().map((media) => (
