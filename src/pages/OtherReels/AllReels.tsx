...
const AllReels: React.FC = () => {
  ...
  const fetchReels = async (params = {}, nextPageUrl = null, perPage = 25) => {
    ...
    try {
      const url = nextPageUrl || constants.BASE_URL + '/otherreels';
  ...
  const deleteReel = async (id: number) => {
    ...
    try {
      await axios.post(constants.BASE_URL + '/otherreel/delete', { id });
  ...
  return (
    <>
      <Breadcrumb pageName="All Other Reels" backLink="/" createLink='/otherreels/create' />
