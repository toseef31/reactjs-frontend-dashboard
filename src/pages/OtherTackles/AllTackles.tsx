...
const AllTackles: React.FC = () => {
  ...
  const [othertackless, setTackles] = useState<any[]>([]);
  ...
  const fetchTackles = async (params = {}, nextPageUrl = null, perPage = 25) => {
    ...
    try {
      const url = nextPageUrl || constants.BASE_URL + '/othertackles';
  ...
  const deleteTackles = async (id: number) => {
    ...
    try {
      await axios.post(constants.BASE_URL + '/othertackles/delete', { id });
      // Update state to remove the deleted othertackles
      setTackles(prevTackles => prevTackles.filter(othertackles => othertackles.id !== id));
